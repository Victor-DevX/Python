...
def live_neighbors(grid, row, col):
    ...
    count = 0                                              # счётчик живых соседей его возвращаем в return
    rows = len(grid)                                       # нужно, чтобы не выйти 
    cols = len(grid[0])                                    # за границы массива
    if row >= 1:                                           # если клетка не в первой строке, можно смотреть строку выше
        min_r = row - 1                                    # если в первой — выше уже нельзя
    else:
        min_r = 0
    max_r = row + 1 if row < rows - 1 else row             # если клетка не в последней строке, можно смотреть ниже
    min_c = col - 1 if col >= 1 else 0
    max_c = col + 1 if col < cols - 1 else col
#    print(f'{row} {col}')
    for idx_y in range(min_r, max_r + 1):                  # проход по строкам вокруг клетки
        for idx_x in range(min_c, max_c + 1):              # проход по столбцам вокруг клетки
            #print(f'The value of grid [{idx_x}][{idx_y}] is {grid[idx_y][idx_y]}')
            if idx_y == row and idx_x == col:              # пропуск клетки если она не считается соседней
                continue
            if grid[idx_y][idx_x] > 0:                     # если соседняя клетка живая, то count +1
                count += 1
    return count
...
#    print(new_grid)                                                # создаём НОВОЕ поле
    for row in range(rows):
        for col in range(cols):                                     # посещаем КАЖДУЮ клетку поля
            live_nb = live_neighbors(grid, row, col)

            if grid[row][col] > 0:                                  # если клетка "жива"
                if live_nb < 2 or live_nb > 3:                      # если клетка не жива
                    new_grid[row][col] = 0
                else:                                               # клетка остаётся живой и возраст увеличивается 
                    new_grid[row][col] = grid[row][col] + 1
            else:                                                   # рождение новой клетки
                if live_nb == 3:
                    new_grid[row][col] = 1
    return new_grid
...
def read_input(filename):
    ...
    grid =[]
    with open(filename, "r") as input_file:             # with: автоматически закроет файл и безопаснее, чем open() вручную
        lines = input_file.readlines()                  # чтение строк
        #print(f'File contents: {lines}')
        for line in lines:
            line = line.strip()                         # убираем \n
            line = line.split(';')                      # разделяем по ; получим вид ["0", "1", "0"] 
            line = [int(elem) for elem in line]         # преобразуем в числа получим вид [0, 1, 0]
            grid.append(line)                           # добавляем в поле
    return grid
...
#    pass
    ...
    with open(filename, 'w') as f:                                  # открываем файл на запись (СТИРАЕТ старый файл!)
        for row in grid:                                            # запись строк
            f.write(';'.join(str(cell) for cell in row) + '\n')     # cell → str(cell) объединение через ;
...
def write_png(grid, filename):
    ...
    rows, cols = len(grid), len(grid[0])
    # размер изображения с учетом размера клетки и расстояния между клетками
    ...
    im = Image.new("RGB", (width, height), (0, 0, 0))      # создадим чёрный фон
    #im.show()
    ...
    for row in range(rows):
        for col in range(cols):
            if grid[row][col] > 0:
                x1 = BORDER_WIDTH + col * (CELL_SIZE + BORDER_WIDTH)    # смещение + шаг по сетке
                y1 = BORDER_WIDTH + row * (CELL_SIZE + BORDER_WIDTH)
                x2 = x1 + CELL_SIZE
                y2 = y1 + CELL_SIZE

                draw.rectangle(                                         # рисуем зелёный квадрат
                    [(x1, y1), (x2, y2)],
                    fill=LIVE_COLOR
                )
    im.save(filename)                                                   # cохранение PNG
...
try:                                                       # Если файл не найден — берём дефолтный
...
if DEBUG:
    ...
    for gen in range(generations):                              # каждая итерация = одно поколение
        grid = model(grid)                                      # обновляем поле
                                                                # сохраняем результат
        write_output(
            grid,
